...
export const config: WebdriverIO.Config = {
  ...
  capabilities: [{
    maxInstances: 1,
    'tauri:options': {
      application: './src-tauri/target/debug/just-clipboard-manager'
    },
  }],
  ...
  cucumberOpts: {
    ...
    require: [
      './e2e-tests/steps/given.ts',
      './e2e-tests/steps/when.ts',
      './e2e-tests/steps/then.ts',
    ],
  ...
  onPrepare: function (config, capabilities) {
    spawnSync(
      'npm',
      ['run', 'tauri', 'build', '--', '--debug', '--no-bundle', '--config src-tauri/tauri.e2e.conf.json'],
      {
        cwd: __dirname,
        stdio: 'inherit',
        shell: true,
